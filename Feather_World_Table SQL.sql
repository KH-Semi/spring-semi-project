...
-- 일촌평 테이블
CREATE TABLE ILCHON_CONTENT 
(
    FROM_MEMBER_NO NUMBER NOT NULL,
    TO_MEMBER_NO NUMBER NOT NULL,
    
    COMMENT NVARCHAR2(30) NOT NULL );

 SELECT * FROM MEMBER;
   
SELECT * FROM USER_TABLES WHERE TABLE_NAME = 'ILCHON_COMMENT';


UPDATE ILCHON SET
IS_ILCHON = 'Y'
WHERE FROM_MEMBER_NO = 8
AND TO_MEMBER_NO = 16;
...
ALTER TABLE "ILCHON" 
ADD (
    "FROM_COMMENT" NVARCHAR2(30),
    "TO_COMMENT" NVARCHAR2(30)
);
SELECT * FROM ILCHON;
SELECT * FROM MEMBER;

UPDATE ILCHON SET IS_ILCHON = 'Y'  
WHERE FROM_MEMBER_NO = 8;
...
>>>>>>> e26c50e019dcf94e041a5746a4f5c72c04588801
...
SELECT * FROM ILCHON 
WHERE IS_ILCHON = 'Y' 
AND (FROM_MEMBER_NO IN (23, 8) OR TO_MEMBER_NO IN (23, 8))
ORDER BY FROM_MEMBER_NO, TO_MEMBER_NO;
...
SELECT * FROM ILCHON WHERE FROM_MEMBER_NO = 8 AND TO_MEMBER_NO = 23;
COMMIT;
...
SELECT 
    CASE 
        WHEN FROM_MEMBER_NO = 8 THEN 'FROM'
        WHEN TO_MEMBER_NO = 23 THEN 'TO'
    END AS MY_POSITION
FROM ILCHON
WHERE (FROM_MEMBER_NO = 8 OR TO_MEMBER_NO = 23)
  AND IS_ILCHON = 'Y';


SELECT 
    CASE 
        WHEN FROM_MEMBER_NO = 8
        THEN TO_MEMBER_NO
        ELSE FROM_MEMBER_NO
    END AS ILCHON_MEMBER_NO,

    CASE 
        WHEN TO_MEMBER_NO = 23
        THEN FROM_COMMENT  -- 상대가 남긴 코멘트
        WHEN FROM_MEMBER_NO = 8 
        THEN TO_COMMENT
    END AS COMMENT_FROM_OTHER
    
FROM ILCHON
WHERE (FROM_MEMBER_NO = 8 OR TO_MEMBER_NO = 23)
  AND IS_ILCHON = 'Y'
  AND (
      (TO_MEMBER_NO = 23 AND FROM_COMMENT IS NOT NULL)
   OR (FROM_MEMBER_NO = 8 AND TO_COMMENT IS NOT NULL)
);

-- 현재 ILCHON 테이블의 실제 데이터 확인
SELECT FROM_MEMBER_NO, TO_MEMBER_NO, FROM_COMMENT, TO_COMMENT, IS_ILCHON
FROM ILCHON 
WHERE (FROM_MEMBER_NO = 24 OR TO_MEMBER_NO = 24) 
AND IS_ILCHON = 'Y';


UPDATE MEMBER SET
MEMBER_EMAIL = 'gimyeongmin95@gmail.com'
WHERE MEMBER_NAME = '영민2';

COMMIT;

SELECT MEMBER_EMAIL, MEMBER_NAME, MEMBER_DEL_FL 
FROM MEMBER 
WHERE MEMBER_EMAIL = 'gimyeongmin95@gmail.com';
